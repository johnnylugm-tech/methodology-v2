...
## v5.28 Server-Side Enforcement 架構
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  Local Hook（可被繞過）──▶ GitHub Actions ──▶ Server Enforcement│
│                                                              │
│  PR/Merge 請求時，強制執行五關：                                │
│                                                              │
│  1. Constitution Check → 規則定義是否正確                      │
│  2. Policy Engine      → 政策是否被遵守                        │
│  3. Quality Gate       → 品質分數 >= 90                       │
│  4. Security Scan       → 安全分數 >= 95                       │
│  5. Registry Audit      → 執行步驟是否完整                      │
│                                                              │
│  全部通過 → ✅ Ready to Merge                                 │
│  任何失敗 → ❌ Blocked                                        │
└─────────────────────────────────────────────────────────────┘
...
| 漏洞 | Local Hook | Server-Side |
|------|------------|-------------|
| `--no-verify` 繞過 | ❌ 可繞過 | ✅ 無法繞過 |
| Hook 可被刪除 | ❌ 失效 | ✅ 不受影響 |
| 只檢查 local | ❌ 危險 | ✅ 隔離環境 |
| 環境不一致 | ❌ 依賴 local | ✅ 統一 CI 環境 |
