...
class GraphRunner:
    ...
    def __init__(self, compiled_graph: Any, config: Optional[ExecutionConfig] = None) -> None:
        """
    ...
    def _invoke_sync_wrapper(
        self, initial_state: dict[str, Any], callbacks: ExecutionCallbacks
    ) -> dict[str, Any]:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(asyncio.run, self._invoke_async(initial_state, callbacks))
    ...
    def resume(self, checkpoint_id: str, resume_input: dict[str, Any]) -> dict[str, Any]:
        """
    ...
    async def _resume_async(
        self, checkpoint: Checkpoint, resume_input: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    def _execute_node(self, node_name: str, state: dict[str, Any]) -> ExecutionResult:
        """
    ...
    def _handle_node_error(
        self, node_name: str, error: Exception, state: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        recovered_state = {
            **state,
            "__last_error__": {"node": node_name, "type": error_type, "message": error_msg},
        }

        return recovered_state

    def _make_langchain_callback(self, callbacks: ExecutionCallbacks) -> Any:
        """
        ...
        class ExecutionCallbackHandler(BaseCallbackHandler):
            ...
            def on_chain_start(
                inner_self, serialized: dict[str, Any], inputs: dict[str, Any], **kwargs: Any
            ) -> None:
                # LangChain calls this at various levels; we filter for node-level
                pass

            def on_chain_end(inner_self, outputs: dict[str, Any], **kwargs: Any) -> None:
                pass

            def on_node_start(inner_self, name: str, **kwargs: Any) -> None:
                inner_self._node_stack.append(name)
                if callbacks.on_node_start:
                    callbacks.on_node_start(name, kwargs.get("inputs", {}))

            def on_node_end(inner_self, name: str, **kwargs: Any) -> None:
            ...
            def on_tool_error(inner_self, error: Exception, **kwargs: Any) -> None:
            ...
            def on_text(inner_self, text: str, **kwargs: Any) -> None:
                pass
    ...
    def _reset_profiling(self) -> None:
        ...
        self._profiling_data = {"nodes": {}, "errors": [], "start_time": time.time()}
    ...
    def checkpoint_current_state(
        ...
        checkpoint_data = {"state": state, "metadata": metadata or {}, "timestamp": time.time()}
